...
function ensureFoodGuides(){
  ...
  if(storedVersion<FOOD_GUIDE_VERSION){
    const custom=current.filter(x=>!curatedIds.has(x.id)&&!/^venice-09(15|16)-coffee-/.test(x.id));
...
function renderGuide(){
  ...
  const renderPlaces=()=>{
    ...
    if(sorted.length&&['Venice','Florence','Rome'].includes(activeCity)&&!placeSearch){
      const dateRange=activeCity==='Venice'?'September 15–16':activeCity==='Florence'?'September 17–19':'September 20–23';
      ...
      const dayTitle=date=>({
        '2026-09-15':'Arrival Day','2026-09-16':'Tour Day','2026-09-17':'Arrival Day','2026-09-18':'Museum Day','2026-09-19':'Pisa & Wine Tour','2026-09-20':'Arrival Day','2026-09-21':'Colosseum Day','2026-09-22':'Vatican Day','2026-09-23':'Final Rome Day'
      })[date]||'';
      const bucket=foodBucketState(activeCity);
      qs('#guideCards').innerHTML=`<section class="venice-food-intro"><div><span>🇮🇹</span><div><div class="focus-label">${activeCity.toUpperCase()} FOOD COMPANION</div><h2>${activeCity}</h2><p>Your day-by-day food plan for ${dateRange}.</p></div></div></section>
        ...
        <section class="food-companion-box"><h3>🏆 ${activeCity} Signature Experiences</h3>${extras.signatures.map(x=>`<div>${escapeHTML(x)}</div>`).join('')}</section>
        <section class="food-companion-box skip"><h3>🚫 ${activeCity} Skip List</h3>${extras.skip.map(x=>`<div>❌ ${escapeHTML(x)}</div>`).join('')}</section>
        <section class="food-passport"><div class="food-passport-head"><span>⭐</span><div><h3>If You Only Do Five Food Things</h3><small>Your essential ${activeCity} food list</small></div></div>${extras.five.map((item,i)=>`<label class="food-passport-item"><input type="checkbox" data-food-bucket="${escapeHTML(item)}" ${bucket[item]?'checked':''}><span><b>${i+1}.</b> ${escapeHTML(item)}</span></label>`).join('')}</section>`;
